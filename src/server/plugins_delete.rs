...
pub(crate) async fn delete_plugin_handler(
    ...
    #[allow(unreachable_code)]
    {
        ...
        let on_disk_builtin =
            plugins_yaml::is_plugin_builtin(data_dir, &name, &plugins_yaml::PluginYamlType::Tool)
                || plugins_yaml::is_plugin_builtin(
                    data_dir,
                    &name,
                    &plugins_yaml::PluginYamlType::Platform,
                )
        ...
        if yaml_source == Some("built-in") {
            if let Some((yaml_type, _)) = yaml_info.as_ref() {
                if let Ok(true) = plugins_yaml::remove_entry(data_dir, yaml_type, &name) {
                    tracing::info!(
                        "Remove: removed YAML entry for built-in plugin '{}' (source not on disk)",
                        name
                    );
                }
            }
        ...
        if yaml_source == Some("remote") || (yaml_source.is_none() && on_disk_remote) {
            ...
            if remote_path.exists() && remote_path.is_dir() {
                match std::fs::remove_dir_all(remote_path) {
                    ...
                    Err(e) => {
                        tracing::warn!(
                            "Remove: failed to remove .remote/ directory for '{}': {:?}",
                            name,
                            e
                        );
                    }
            ...
            || (yaml_source.is_none() && on_disk_bundled)
        {
            ...
            if plugin_path.exists() && plugin_path.is_dir() {
                match std::fs::remove_dir_all(plugin_path) {
                    Ok(()) => {
                        tracing::info!(
                            "Remove: removed workspace directory for bundled plugin '{}'",
                            name
                        );
                        removed = true;
                    }
                    Err(e) => {
                        tracing::warn!(
                            "Remove: failed to remove workspace directory for '{}': {:?}",
                            name,
                            e
                        );
                    }
            ...
            if data_plugin_path.exists() && data_plugin_path.is_dir() {
                ...
                tracing::info!(
                    "Remove: removed data directory for bundled plugin '{}'",
                    name
                );
            }
        ...
        if let Some((yaml_type, source)) = yaml_info {
            if let Ok(true) = plugins_yaml::remove_entry(data_dir, &yaml_type, &name) {
                tracing::info!(
                    "Remove: removed orphaned YAML entry for plugin '{}' (source={}, not on disk)",
                    name,
                    source
                );
                removed = true;
...
pub(crate) async fn handle_remove_by_source(
    ...
    match source {
        "built-in" => {
            ...
            let on_disk = plugins_yaml::is_plugin_builtin(
                data_dir,
                name,
                &plugins_yaml::PluginYamlType::Tool,
            ) || plugins_yaml::is_plugin_builtin(
                ...
                if let Ok(Some(entry)) = plugins_yaml::get_entry(data_dir, pt, name) {
                    if entry.source == "built-in" {
                        if let Err(e) = plugins_yaml::remove_entry(data_dir, pt, name) {
                            tracing::warn!(
                                "[plugins] Failed to remove built-in phantom YAML: {:?}",
                                e
                            );
                        }
                        removed = true;
