...
pub struct WorkflowRole {
    ...
    /// Toolset id (`config/toolsets.yml`) applied when this role runs a
    /// thread. OPTIONAL: absent means "this role defines no toolset", so the
    /// next level of the chain (`workflow > task > channel > profile`)
    /// applies. This supersedes the old per-role `allowed_tools` intersection.
...
pub struct Workflow {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub template: Option<String>,
    /// Optional workflow-level toolset id (`config/toolsets.yml`). Fallback
    /// for every role that does not define its own `toolset`: the one
    /// effective toolset chain is
    ...
    pub fn validate(&self) -> Result<(), WorkflowConfigError> {
        for (key, workflow) in &self.workflows {
            ...
            for (role_key, role_def) in &workflow.roles {
                ...
                // A role `toolset` must be a non-blank toolset id.
                if let Some(toolset) = &role_def.toolset {
                    if toolset.trim().is_empty() {
                        return Err(WorkflowConfigError::Invalid {
                            key: key.clone(),
                            message: format!("role '{role_key}' toolset must not be blank"),
                        });
                    }
                }
            }
...
pub struct ResolvedWorkflowRole {
    ...
    pub plan_mode: Option<String>,
    pub retries: Option<u32>,
    /// Effective toolset id after resolution: the role's own `toolset`, or
