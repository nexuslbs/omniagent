...
fn generate_initial_prompt_tool() -> McpTool {
    McpTool {
        ...
        description: "Generate system prompt and planning prompt for a new conversation. \
...
fn compact_messages_tool() -> McpTool {
    McpTool {
        ...
        description: "Compact old assistant messages in a conversation to save tokens. \
        ...
        handler: Arc::new(|args: Value, _ctx: AppContext| {
            Box::pin(async move {
                let messages_arr = match args["messages"].as_array() {
                    ...
                    None => return Ok(McpToolResult {
                        call_id: String::new(),
                        content: "Missing required argument: 'messages' (array of ChatMessage)".to_string(),
                        is_error: true,
                    }),
                };
                ...
                let mut messages: Vec<crate::llm::ChatMessage> = match serde_json::from_value(
