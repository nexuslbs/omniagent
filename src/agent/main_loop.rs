...
Previous plan:\n{}",
    ...
    // Every thread - ordinary operator chats included - is budgeted from the
    ...
    for _turn in 0..max_llm_calls {
        ...
        if current_iter >= iter_limit {
            ...
            if !response.tool_calls.is_empty() {
                // Iteration budget exhausted while the model still requested tools:
                // record the interruption without executing the pending tools.
...
#[cfg(test)]
mod iteration_budget_tests {

    /// Regression guard for the removed hidden interactive cap. Every thread -
    /// including an ordinary operator thread (cause=user, no delegated
    /// kanban/schedule task) - is budgeted from the plugin setting via
    /// `queries::max_iterations_for_plan`: the plan or no-plan budget and
    /// nothing else. No hidden MIN may be reintroduced in this module.
    #[test]
    fn interactive_operator_thread_keeps_full_plan_budget() {
        let src = include_str!("main_loop.rs");
        for forbidden in [
            concat!("interactive_max", "_iterations"),
            concat!("interactive_iter", "_limit"),
            concat!("is_interactive", "_thread"),
        ] {
            assert!(
                !src.contains(forbidden),
                "hidden interactive-cap machinery `{forbidden}` was reintroduced"
            );
        }
        assert!(
            src.contains("max_iterations_for_plan(&cfg.config_snapshot()"),
            "every thread must be budgeted from the plugin plan/no-plan setting"
        );
    }
}
