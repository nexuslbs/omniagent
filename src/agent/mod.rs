...
pub(crate) mod context_compactor;
pub mod helpers;
...
impl Agent {
    ...
    pub fn new(
        ...
        // literal models.yml headers apply plus the `fallback` of any typed
        // header that declares one (typed channel/profile values with neither
        // context nor fallback are skipped loudly). Still routed through the
        // shared resolver, so a code-less (plugin: false) provider receives
        // every header it declares in models.yml, exactly like the per-thread
        // client.
        ...
        let llm_config = crate::llm::LLMConfig {
            ...
            supports_reasoning: crate::llm::PROVIDER_METADATA
                ...
                .unwrap_or(false),
            // Resolved above: literal (and typed-fallback) models.yml headers.
            extra_headers,
        };
    ...
    pub async fn run(self, cancel_tokens: Arc<Mutex<HashMap<String, CancellationToken>>>) {
        ...
        // The recovery phase needs the data dir (for the startup skip logic)
        ...
        tokio::select! {
                    ...
                    if let Err(e) = queries::skip_thread(&cfg.pool, active_id).await {
                        ...
                        let channel =
                            crate::db::channels::get_channel_by_id(&cfg.pool, &channel_id)
                                ...
                                .ok()
                                .flatten();
                        crate::agent::helpers::enqueue_status_reaction(
                            &cfg.ctx,
                            &cfg.pool,
                            active_id,
                            None,
            ...
            _ = async {
                ...
                if let Ok(true) = queries::is_channel_closed(&cfg.pool, &channel_id).await {
                    ...
                    // Channel is closed: its threads are skipped with no
                    // status reaction (no deliverable platform target exists),
                    // so the DB-only skip path is used here.
