...
pub async fn run_cli(channel_name: String, profile_name: String, model: Option<String>, provider: Option<String>, data_dir: &str) -> Result<()> {
    ...
    let planning_mode = crate::db::types::resolve_thread_planning_mode(
    ...
    crate::db::types::set_thread_system(&pool, thread_id).await?;
    ...
    loop {
        ...
        match input.as_str() {
            ...
            cmd if cmd.starts_with("/subscribe ") => {
                ...
                match crate::db::types::get_channel_by_name(&pool, &name).await? {
                    Some(ch) => {
                        let sub_id = crate::db::types::add_subscription(
            ...
            cmd if cmd.starts_with("/unsubscribe ") => {
                ...
                match crate::db::types::get_channel_by_name(&pool, &name).await? {
                    Some(ch) => {
                        let removed = crate::db::types::remove_subscription(
                            &pool, ch.id, "cli", &session_id,
                        ).await?;
                        if removed {
                            println!(
            ...
            "/subscriptions" => {
                let subs = crate::db::types::get_subscriptions_for_subscriber(
                    &pool, "cli", &session_id,
                ).await?;
                if subs.is_empty() {
                    ...
                    for sub in &subs {
                        let ch_name = crate::db::types::find_channel_by_id(&pool, sub.channel_id)
                            .await?
                            .map(|c| c.name)
        ...
        if session_has_channel {
            if let Some(ch) = crate::db::types::find_channel_by_id(&pool, current_channel_id).await? {
        ...
        crate::db::types::create_thread_with_cause(
            &pool,
            "user",
            current_channel_id,
            &profile_name,
...
async fn ensure_cli_channel(
    ...
    if let Some(channel_db) = existing {
        ...
        let channel = crate::db::types::Channel {
            id: channel_db.id,
    ...
    // Create new channel
    let new_channel = crate::db::types::create_channel(
        pool,
...
async fn get_or_create_thread(pool: &PgPool, p: ThreadLookupParams) -> Result<i64> {
    ...
    let root_msg = crate::db::types::MessageNew {
        thread_id: thread.id,
...
async fn poll_for_response(
    ...
    loop {
        ...
        if let Some(ch) = crate::db::types::find_channel_by_id(pool, channel_id).await? {
            if ch.resource_identifier.as_deref() != Some(session_id) {
                println!(
                    "\n[ERR_SESSION_NO_CHANNEL] This session no longer has a channel. \
                     Use /channel to claim one.\n"
        ...
        let subs = crate::db::types::get_subscriptions_for_subscriber(pool, "cli", session_id).await?;
        for sub in &subs {
            let summaries = crate::db::types::get_summaries_since(pool, sub.channel_id, last_seen_summary).await?;
        ...
        match choice.trim() {
            "1" => {
                ...
                let name = loop {
                    ...
                    if crate::db::types::validate_channel_name(&name) {
                        // Check uniqueness (only among 'cli' platform channels)
                        let existing =
                            crate::db::types::get_channel_by_platform_name(pool, "cli", &name).await?;
                        if existing.is_some() {
                            println!(
            ...
            "2" => {
                ...
                let all_channels = crate::db::types::find_all_channels(pool).await?;
...
async fn handle_action_command(pool: &PgPool, input: &str) -> anyhow::Result<()> {
    ...
    if let Some(num_str) = action_arg {
        if num_str.is_empty() {
            // Just "/action " — list
            let actions = crate::db::types::list_actions(pool).await?;
            if actions.is_empty() {
...
async fn handle_usage_command(pool: &PgPool) -> anyhow::Result<()> {
    let stats = crate::db::types::get_channel_usage_stats(pool).await?;
...
async fn handle_cli_model_command(
    ...
    let parsed = match crate::commands::parse_model_command(input) {
        Ok(cmd) => cmd,
        Err(e) => {
            println!("Error: {}", e);
            return Ok(());
    ...
    match parsed.action {
        ...
        crate::commands::ModelAction::Set { provider, model } => {
            ...
            if let Some(ref p) = provider {
                if !p.is_empty() {
                    match crate::commands::validate_provider(pool, p).await {
                        Ok(_) => {}
                        Err(e) => {
                            println!("Error: {}", e);
                            return Ok(());
...
async fn handle_cli_profile_command(
    ...
    let parsed = match crate::commands::parse_profile_command(input) {
        Ok(cmd) => cmd,
        Err(e) => {
            println!("Error: {}", e);
            return Ok(());
    ...
    match parsed {
        ...
        crate::commands::ProfileCommand::Set(ref name) => {
            ...
            crate::commands::handle_profile_set(pool, channel_id, name).await?;
            println!("Profile set to '{}'.", name);
        }
        crate::commands::ProfileCommand::Reset => {
            crate::commands::handle_profile_set(pool, channel_id, "default").await?;
